...
#. Implement ``calculate_jacobian`` in ``reacher_kinematics.py`` using the finite differencing method we covered in lecture
#. Implement ``calculate_inverse_kinematics`` in ``reacher_kinematics.py``
